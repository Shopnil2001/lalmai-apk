        ...
        if (firebaseUser) {
          try {
            ...
            if (userDoc.exists()) {
              ...
              // Run migration check in background for admin sessions
              if (profile.role === 'Super Admin' || profile.role === 'Admin') {
                const { dbService } = require('../services/dbService');
                dbService.runMigrationCheck().catch((err: any) => {
                  console.error('[Migration] Failed to run migration check:', err);
                });
              }
            } else {
