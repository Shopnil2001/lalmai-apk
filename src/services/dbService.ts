...
export const dbService = {
  ...
  createUserByAdmin: async (profile: Omit<UserProfile, 'uid' | 'joinedAt'>, password?: string): Promise<UserProfile> => {
    if (hasRealFirebase && db) {
      ...
      const cred = await createUserWithEmailAndPassword(secAuth, profile.email, password || 'password');
      ...
      if (profile.phone) {
        ...
        if (cleanPhone) {
          await setDoc(doc(db, 'phone_mappings', cleanPhone), { email: profile.email.toLowerCase(), uid });
        }
      }
      
      return userProfile;
    } else {
      return mockDB.createUserByAdmin(profile, password);
    }
  },
  ...
  injectSeedData: async (): Promise<void> => {
    if (hasRealFirebase && db && auth) {
      ...
      for (const user of SEED_USERS) {
        let realUid = '';
        try {
          // Attempt to create Auth user (password is default 'password' or '123456')
          ...
          if (error.code === 'auth/email-already-in-use') {
            ...
            try {
              ...
              console.error(`Failed to sign in as ${user.email} to retrieve UID`, signInError);
        ...
        if (user.phone) {
          ...
          if (cleanPhone) {
            await setDoc(doc(db!, 'phone_mappings', cleanPhone), { email: user.email.toLowerCase(), uid: realUid });
          }
        }
      }
