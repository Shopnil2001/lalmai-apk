...
export const authService = {
  ...
  sendPhoneOTP: async (phoneNumber: string, recaptchaVerifier: any): Promise<any> => {
    ...
    if (hasRealFirebase && auth) {
      // Phone auth on native Android/iOS works without recaptchaVerifier.
      // It uses the native Firebase SDK which handles reCAPTCHA internally.
      // recaptchaVerifier is only needed for web (react-native-web) which we don't use.
      if (!cleanPhone.startsWith('+')) {
        throw new Error('Please enter your phone number with the country code (e.g. +8801700000000).');
      }
      try {
      ...
      // MOCK Phone OTP
      const users = await mockDB.getUsers();
      const user = users.find(u => u.phone.trim() === cleanPhone);
      ...
      console.log(`[MOCK OTP] Sent code '123456' to ${cleanPhone}`);
      
      // Return a simulated confirmationResult object
      return {
  ...
  // Verify Phone OTP — also handles first-time phone sign-up
  verifyPhoneOTP: async (confirmationResult: any, code: string): Promise<UserProfile> => {
    ...
    if (hasRealFirebase && db) {
      ...
      let profile: UserProfile;
      if (!userDoc.exists()) {
        // First-time phone sign-up: create a pending profile
        profile = {
          uid,
          name: '',
          email: '',
        ...
        profile = userDoc.data() as UserProfile;
        // Guard: if this account was registered via a different method, block login
        if (profile.authProvider && profile.authProvider !== 'phone') {
          await signOut(auth!);
          throw new Error(
            `This phone number's account was registered using ${
              profile.authProvider === 'email' ? 'email & password' : 'Google'
            }. Please sign in using that method instead.`
          );
        }
      }
      ...
      // For mock mode, the confirm() method in sendPhoneOTP handles session saving
      const session = await authService.getCurrentSession();
      if (!session) throw new Error('Failed to resolve authenticated session.');
      return session;
    }
  },
